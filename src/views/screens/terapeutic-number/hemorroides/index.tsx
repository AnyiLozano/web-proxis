...
const Hemorroides = () => {
    ...
    const { lines, width, bibliografias, hemo } = useTerapeuticNumber();

    return (
        <React.Fragment>
            <StyledImage src={hemo?.image?.image} alt="terapeutica" />
            <StyledTerapeuticNumberSection background="http://localhost/api-proxis/wp-content/uploads/2021/12/Recurso 4.png">
                <Container>
                    ...
                    <Grid item lg={12} className="text-center">
                        <StyledTitulo1>{hemo?.description.description}</StyledTitulo1>
                    </Grid>
                </Container>
            </StyledTerapeuticNumberSection>
            <StyledTerapeuticNumberSection1 background="http://localhost/api-proxis/wp-content/uploads/2021/12/Recurso-8.png">
                <Container>
                    <Grid item lg={12}>
                        <Grid container>
                            {
                                _.map(hemo?.lines, (item: any, index: any) => {
                                    if(width >= 1024){
                                        if(index%2 === 0){
                                            return (
                                                <React.Fragment>
